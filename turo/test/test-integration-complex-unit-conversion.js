...
test("Nth power", function (t) {
  ...
  // Invalid statements.
  t.ok(turo.evaluate("(3 m)^(2 m)").expressionErrors(), "(3 m)^(2 m) is invalid");
  t.ok(turo.evaluate("(3 m^3)^(1.5)").expressionErrors(), "(3 m^3)^(1.5) is invalid");
  t.ok(turo.evaluate("(3 m^3)^(.5)").expressionErrors(), "(3 m^3)^(.5) is invalid");
...
test('Unit prefix ops', function (t) {
  ...
  t.ok(turo.evaluate('cos(0 m)').expressionErrors().length > 0, 'cos(0 m)');
...
test('Simple unit conversion based on unitScheme', function (t) {
  ...
  turo.reset();
  turo.evaluate('include "app"');

  var prefs = turo.prefs();
  prefs.useUnitRefactor = true;
  prefs.simpleUnits = true;
  t.equal(turo.evaluate("36 inch").valueToString(), "36 inches", "36 inches");

  prefs.unitScheme = 'Imperial';
  t.equal(turo.evaluate("36 inch").valueToString(), "36 inch", "36 inch");

  prefs.unitScheme = 'Metric';
  t.equal(turo.evaluate("36 inch").valueToString(), "91.44 cm", "36 inch in Metric");
