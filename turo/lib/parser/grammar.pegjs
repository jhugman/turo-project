...
////////////////////////////////////////////////////////////////////////////////
StatementList = _ list:(Statement? StatementSeperator)* {
  var statements = [];
...
StatementSeperator = _ ";" _

////////////////////////////////////////////////////////////////////////////////
EditorText
  = lines:EditorTextLine*
    {
...
FirstParseStatement
  = ImportStatement
  / FirstParseUnitDefinition
  / FirstParseVariableDefinition

/*
*/

FirstParseUnitDefinition
  = "unit" __ (num:NumberLiteral _)? unit:UnitDefinitionLeft colon:(_ UnitAssignmentLiteral)?
    {
      // unit = { id, alternatives, schemes }

      if (colon) {
        // Brand new unit
        parseContext.scope.addUnit(unit, unit.id, unit.alternatives);
      } else {
        // New alternative names for an existing unit
        parseContext.scope.addUnit(unit, null, unit.alternatives);
      }
    }

FirstParseVariableDefinition
  = ('const' __)? id:IdentifierLiteral _ AssignmentLiteral
    {
      parseContext.scope.addVariable(id, {});
    }
...
// unit N Newton (SI) : Force, 1kg m/s^2
// unit psi (imperial) : Pressure, 1 lb/inch^2
// unit m (metric SI) : Length
// unit 2.2 lb (imperial) : kg


UnitDefinitionStatement = "unit" __ unit:UnitDefinition {
  return new ast.StatementNode("UnitDefinition", unit);
}

UnitDefinition =
    unit:NewUnitDefinition {
      parseContext.scope.addUnit(unit, unit.name, unit.alternatives);
      return unit;
    }
  / unit:UnitSchemeAnnotation {
      return unit;
    }

UnitSchemeAnnotation = 
  unitNode:UnitIdentifier alternatives:AlternativeUnitNames _ schemes:UnitSchemes? {
    // Unsure if this the right thing to do: 
    // allow both alternatives and schemes to be optional.
    if (schemes) {
      units.addUnitSchemes(unitNode.unit, schemes);
    }
    if (alternatives.length) {
      units.addUnitAliases(unitNode.unit, alternatives);
      parseContext.scope.addUnit(unitNode.unit, null, alternatives);
    }
...
UnitAssignmentLiteral
  = AssignmentLiteral
  / ":"

UnitDefinitionLeft =
  ids:UnitIdentifiers _ schemes:UnitSchemes? {
    return {
      id: ids[0],
      alternatives: ids[1],
      schemes: schemes
...
UnitIdentifiers = unitName:NewUnitIdentifier alternatives:AlternativeUnitNames {
  return [unitName, alternatives];
}

AlternativeUnitNames = alternatives:(__ NewUnitIdentifier)* {
  var alts = [];
...
UnitSchemes =  ParensOpen schemeLiterals:(_ UnitScheme)* _ ParensClose {
...
OptionalDimensionIdentifier = dimension:DimensionIdentifier _ ',' {
  return dimension;
}

DimensionIdentifier "dimensionName" = dimension:IdentifierLiteral

NewUnitIdentifier "newUnitName" = unitName:UnitIdentifierLiteral

UnitScheme "unitScheme" = unitScheme:IdentifierLiteral
...
ConstDefinition = "const" __ variable:VariableDefinition {
...
NumericalExpression =
  ...
  {
    if (operation) {
      return new ast.BinaryNode(lhs, operation[3], operation[1]);
    }
    return lhs;
  }

ComparatorLiteral "comparator"
  = "!="
  / "<="
  / "=="
  / ">="
...
NumbericalExpressionUnitConversion =
...
UnitConversion = _ op:UnitConversionNode _ unitNode:UnitExpression {
  return [op, unitNode];
}
...
AdditiveExpression
  = head:MultiplicativeExpression
    ...
    {
      return helper.unpackLeftBinaryOperations(head, tail);
    }
...
AdditiveOperatorLiteral "plusMinus"
  = "+"
  / "-"
  / "||"
...
MultiplicativeExpression
  = head:MultiplactionOperand
    tail:(_ MultiplicativeOperator _ MultiplactionOperand)*
    {
      return helper.unpackLeftBinaryOperations(head, tail);
    }
...
MultiplactionOperandWithoutUnit
   = prefixes:(AdditiveOperator _)*
     operand:NamedBinaryOperation
   {
    return helper.unpackUnaryOperations(operand, prefixes, true);
   }


////////////////////////////////////////////////////////////////////////////////
NamedBinaryOperation =
  head:NamedPostfixUnaryOperation
  tail:(_ NamedBinaryOperator _ NamedPostfixUnaryOperation)*
  {
    return helper.unpackRightBinaryOperations(head, tail);
  }
...
SpecialNamedBinaryOperator 
    = "^" 
    / "√"

////////////////////////////////////////////////////////////////////////////////

NamedPrefixUnaryOperation =
  prefixes: (NamedPrefixOperator _)*
     value: ValueOrParens
  {
    value = helper.unpackUnaryOperations(value, prefixes, true);
    //UnaryOperationNode
    return value;
  }
...
SymbolTablePostfixOperator "postfixOp" 
  = "!" !"="
    {
      return "!";
    }
  ...
  / po:ParensOpen _ expression:NumericalExpression _ pc:ParensClose
    {
      ...
      parens._offsetFirst = offset;
      parens._offsetLast = pc.offset;
      parens.isMissingParensClose = (pc.offset >= inputLength);
      return parens; 
    }
...
Identifier "variable"
  ...
  &{ return parseContext.scope.findVariable(identifier); }
  { 
    return new ast.IdentifierNode(identifier, parseContext.scope);
  }

////////////////////////////////////////////////////////////////////////////////

UnitIdentifierLiteral = SpecialUnitLiteral / IdentifierLiteral
...
Number
  = number:NumberLiteralString
  { 
    var node = new ast.NumberNode(number.literal);
    node.exponent = number.exponent;
    node.integerPart = number.integerPart;
    node.sign = number.sign;
    return node;
  }

NumberLiteral = numberObj:NumberLiteralString {
  return parseFloat(numberObj.literal , 10);
}

NumberLiteralString = sign:AdditiveOperatorLiteral? valueNode:NumberLiteralFromParts exp:ExponentLiteral? {
...
ExponentLiteral = ExponentSign integer:IntegerString {
  return integer;
}
...
IntegerLiteral = string:IntegerString {
...
KeywordLiteral = "unit" / "per" / "const" / "test" 
              / "include" / "units" / "let" / "in"
              / "at" / "of" / "every" 
